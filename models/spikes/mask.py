...
class Mask_s(nn.Module):
    ...
    '''
    def __init__(self, h, w, planes, block_w, block_h, eps=0.4,
                 bias=-1, args=None):
        ...
        # spatial attention
        self.atten_s = nn.Conv2d(planes, planes, kernel_size=3, stride=1, bias=bias>=0, padding=1).cuda()
        if bias>=0:
    ...
    def forward(self, x):
        ...
        #mask = torch.nn.functional.pad(mask_s, (pad_width, pad_width, pad_height, pad_height))
        mask_s = mask_s.repeat(1,1,self.block_w,self.block_h)
